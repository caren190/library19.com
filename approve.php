...
<head>
	...
	<style type="text/css">
		...
		body {
			background-image: url("images/1111(1).jpg");
...
<?php
  ...
  {
    mysqli_query($db,"UPDATE  `issue_book` SET  `approve` =  '$_POST[approve]', `issue` =  '$_POST[issue]', `return` =  '$_POST[return]' WHERE username='$_SESSION[name]' and isbn='$_SESSION[isbn]';");

    mysqli_query($db,"UPDATE books SET quantity = quantity-1 where isbn='$_SESSION[isbn]' ;");
    ...
    while($row=mysqli_fetch_assoc($res))
    {
      if($row['quantity']==0)
      {
        mysqli_query($db,"UPDATE books SET status='not-available' where isbn='$_SESSION[isbn]';");
      }
    }
    ?>
